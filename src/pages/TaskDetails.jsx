  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-6 bg-white rounded-2xl shadow-md mt-6">
      ...
      <div className="flex flex-wrap gap-4 mb-5">
        <span className={`capitalize px-3 py-1 text-sm rounded-full font-medium ${statusColors[task.status]}`}>
          {task.status}
        </span>
        <span className={`px-3 py-1 text-sm rounded-full ${priorityColors[task.priority]}`}>
      ...
      <div className="text-sm text-gray-600 flex justify-between space-y-1 mb-6">
        <p><span className="font-medium">Start Date:</span> {formatDate(task.startDate)}</p>
          ...
          <ul className="space-y-3">
            {task.checklist.map((item, index) => (
              ...
              >
                ...
                <span
                  className={`text-sm ${
                    item.done ? "text-gray-500 text-2xl" : "text-gray-800 text-2xl"
                  }`}
                >
                  {item.text}
                </span>
